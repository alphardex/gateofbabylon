...
首先，我们fork[这个模板](https://codesandbox.io/s/kokomi-js-starter-tjh29w)来创建一个最简单的场景
...
`kokomi.convertGeometryToShape`是一个快捷函数，能自动将three.js的geometry转化为cannon.js所需的shape，不用手动地来定义shape
...
这里的材质用了kokomi.js自带的玻璃材质`GlassMaterial`
...
[![qOpPXQ.png](https://s1.ax1x.com/2022/04/05/qOpPXQ.png)](https://imgtu.com/i/qOpPXQ)
...
[![qOFrf1.gif](https://s1.ax1x.com/2022/04/05/qOFrf1.gif)](https://imgtu.com/i/qOFrf1)
...
在Sketch中实例化Breaker类，定义好所有可粉碎的物体，监听Shooter类的`shoot`事件，当有弹珠射出时给她进行碰撞检测，即监听`collide`事件，将被撞到的物体送给Breaker类来粉碎
...
调用`ConvexObjectBreaker`的`prepareBreakableObject`为mesh创建分割数据，再给`body`绑上`mesh`的id，使其能相互对应
...
1. 获取3个分割所需的数据：网格`mesh`（已有）、碰撞点`poi`（需计算出来）、法线`nor`（也需计算出来）
...
[![qjaWfH.gif](https://s1.ax1x.com/2022/04/06/qjaWfH.gif)](https://imgtu.com/i/qjaWfH)
...
游戏的玩法其实各有千秋：闯关式、无尽模式、限时式等等，本文就采用简单的无尽模式。
...
[![qjjNff.gif](https://s1.ax1x.com/2022/04/06/qjjNff.gif)](https://imgtu.com/i/qjjNff)
...
[![qvovq0.gif](https://s1.ax1x.com/2022/04/06/qvovq0.gif)](https://imgtu.com/i/qvovq0)
...
尽管如此，笔者还是希望读到这里的你能发挥你的想象力，创作出只属于你自己的game。